 ...
 ***************************/
data "aws_lambda_function" "api-summoners" {
  function_name = "api-summoners"
}

resource "aws_api_gateway_resource" "api-summoners-resource" {
  rest_api_id = aws_api_gateway_rest_api.default.id
  parent_id   = aws_api_gateway_rest_api.default.root_resource_id
  path_part   = "summoners"
}

resource "aws_api_gateway_method" "api-summoners-method" {
    rest_api_id = aws_api_gateway_rest_api.default.id
    resource_id = aws_api_gateway_resource.api-summoners-resource.id
    http_method = "ANY"
    authorization = "NONE"
}

resource "aws_api_gateway_integration" "api-summoners-integration" {
    rest_api_id = aws_api_gateway_rest_api.default.id
    resource_id = aws_api_gateway_resource.api-summoners-resource.id
    http_method = aws_api_gateway_method.api-summoners-method.http_method
    integration_http_method = "POST"
    type = "AWS_PROXY"
    uri = data.aws_lambda_function.api-summoners.invoke_arn
}

resource "aws_lambda_permission" "api-summoners-permission" {
    statement_id = "AllowAPIGatewayInvoke"
    action = "lambda:InvokeFunction"
    function_name = data.aws_lambda_function.api-summoners.function_name
    principal = "apigateway.amazonaws.com"
    source_arn = "${aws_api_gateway_rest_api.default.execution_arn}/*/*"
}
